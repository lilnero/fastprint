...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var formResultPreview = "";
  async function formSend(e) {
    e.preventDefault();
    formResult.innerHTML = formResultPreview;
    let error = 0;
    ...
    if (error === 0) {
      ...
      let response = await fetch("https://lilnero.github.io/fastprint/sendmail.php", {
        method: "POST",
        body: formData,
      });
      if (response.ok) {
        ...
        formResult.innerHTML = result.message;
      } else {
        let result = await response.json();
        formResult.innerHTML = result.message;
      }
